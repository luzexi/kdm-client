...
public class LineMesh
{
	...
	public static Mesh MakeMeshLine(List<LineData> _lstLine)
	{
		...
		{
			MeshData _meshdata = MakeMeshData(_lstLine[i].mStartPos, _lstLine[i].mEndPos, _lstLine[i].mWidth, _lstLine[i].mColor);
			lstMeshdata.Add(_meshdata);
		}
		return CombineMeshData(lstMeshdata);
	}

	public static MeshData MakeMeshData(Vector3 _startPos, Vector3 _endPos, float _width, Color _color)
	{
	...
	public static Mesh CombineMeshData(List<MeshData> lst)
	{
		...
		for(int i = 0 ; i<lst.Count ; i++)
		{
			...
			_colors[i*4+0] = lst[i].mColors[0];
			_colors[i*4+1] = lst[i].mColors[1];
			_colors[i*4+2] = lst[i].mColors[2];
			_colors[i*4+3] = lst[i].mColors[3];
		}
