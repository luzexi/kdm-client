...
public class PathNodeManager
{
	...
	public Dictionary<string,PathNode> mDicNameNode = new Dictionary<string, PathNode>();
	...
	public void LoadBinary(string pathfile)
	{
        ...
        for(int i = 0 ; i<nodeCount ; i++)
        {
        	...
        	{
        		if(!mDicNameNode.ContainsKey(node.mName))
        		{
        			mDicNameNode.Add(node.mName, node);
        		}
        		else
        		{
        			Debug.LogError("have the same name in path node");
        			throw new Exception("have the same name in path node");
        		}
        	}
